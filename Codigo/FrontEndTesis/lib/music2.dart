...
class _MusicPageState extends State<MusicPage2> {
  ...
  // Función helper para convertir valores a double de manera segura
  ...
  // Función helper para formatear valores según el tipo de característica
  String _formatValue(String label, dynamic value) {
    if (value == null) return '0';
    
    double numValue = _toDouble(value);
    
    // Song Duration se muestra como número entero (milisegundos)
    if (label == 'Song Duration (ms)') {
      return numValue.toStringAsFixed(0);
    }
    
    // Loudness puede ser negativo, mostrar con 2 decimales
    if (label == 'Loudness') {
      return numValue.toStringAsFixed(2);
    }
    
    // Tempo y Key se muestran como números enteros
    if (label == 'Tempo' || label == 'Key' || label == 'Time Signature' || label == 'Audio Mode') {
      return numValue.toStringAsFixed(0);
    }
    
    // Los demás valores son decimales entre 0 y 1, mostrar con 4 decimales
  ...
  Widget build(BuildContext context) {
    ...
    // Obtener el porcentaje de manera segura (este SÍ es un porcentaje)
    double percentage = _toDouble(songData['Percentage']);
...
IconData getStarIcon(int index, double percentage) {
  if (percentage >= 80) {
    return Icons.star;
